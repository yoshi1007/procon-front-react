...
// ログインAPIをコールする
export const callLoginApi = (login_form_values) => async dispatch => {
...
export const callSubmitApi = (submit_form_values) => async dispatch => {
  await myHttpClient.post(
    `${URL}/submissions`,submit_form_values
  ).then((res)=>{
    console.log(res)
...
export const switchLoginFormModalStatus = (isShowLoginFormModal) => async dispatch => {
  dispatch({ type: SWITCH_LOGIN_FORM_MODAL_STATUS, isShowLoginFormModal })
}

// ログインフォームモーダルを表示・非表示する
export const onClickToCloseSnackbar = () => async dispatch => {
  dispatch({ type: CLOSE_LOGIN_SNACKBAR })
}